...
file. Please see [Dynamic Node Detection](#dynamic_node_detection) for a
thorough discussion on the meaning of the parameter. The default value
is `false`.

### `cluster_monitor_interval`

This optional parameter, meanigful only if `dynamic_node_detection` is
...
changes significantly. In that case, the servers the monitor has been
configured with are _only_ used for "bootstrapping" the monitor. At the
initial startup, the monitor must be told how to get in contact with some
node of the cluster.
...
...
```
As can be seen, the server entries look just like any other server entry,
but to make it stand out for what purpose they are used, they have the
word _bootstrap_ in their name.
...
`maxctrl list servers` looks like
...
`dynamic_node_detection` is desribed [here](#impact_of_dynamic_node_detection).
...
whether the node is added via maxscale or directly using the REST-API
of Columnstore. The only difference is that in the former case,
...
to explicitly remove any server entries (as there never was one in
